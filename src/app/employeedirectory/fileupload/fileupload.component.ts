  ...
  else
  {
    ...
    else if(file.type =="text/plain")
    {
      ...
      fileReader.onload = (e) => {
        ...
        let parsedSstring = data?.split(', ').map((n) => n.split('\t'));
      ...
      this.showSnackbar("Please Enter Excel format files only", "OK"); 
    }
    }
      ...
      dialogRef.afterClosed().subscribe(result => {
        ...
        this.dataSource.data = this.dataSource.data.filter((value,key)=>{
         if(value.socialSecurityNumber == result.data.SocialSecurityNumber){
           ...
           value.citizen = result.data.Citizen;
...
console.log("List",employeDirectoryList);
  ...
  {
    console.log("index",e);
    this.dataSource.data = this.dataSource.data.filter((u:any) => u.socialSecurityNumber !== e.socialSecurityNumber);
